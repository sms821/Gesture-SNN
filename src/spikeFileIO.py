...
class event():
    ...
    def __init__(self, xEvent, yEvent, pEvent, tEvent):
        ...
        self.p -= self.p.min()
        self.xypt = np.zeros((4, len(self.x)))
        self.xypt[0,:], self.xypt[1,:], self.xypt[2,:], self.xypt[3,:] = self.y, self.x, \
                self.p, self.t
    ...
    def toSpikeArray(self, samplingTime=1, dim=None):   # Sampling time in ms
        ...
        '''
        if self.dim == 1:
            if dim is None: dim = ( int(np.round(max(self.p)+1)),
                                    int(np.round(max(self.x)+1)),
                                    int(np.round(max(self.t)/samplingTime+1)) )
            frame = np.zeros((dim[0], 1, dim[1], dim[2]))
        elif self.dim == 2:
            if dim is None: dim = ( int(np.round(max(self.p)+1)),
                                    int(np.round(max(self.y)+1)),
                                    int(np.round(max(self.x)+1)),
                                    int(np.round(max(self.t)/samplingTime+1)) )
            #print(dim)
    ...
    def toSpikeTensor(self, emptyTensor, samplingTime=1, randomShift=False):    # Sampling time in ms
        ...
        elif emptyTensor.shape[0] == 1: # single channel
...
def spikeArrayToEvent(spikeMat, samplingTime=1):
    ...
    if spikeMat.ndim == 3:
        ...
        spikeEvent = np.argwhere(spikeMat > 0)
        #print('event indices: ', spikeEvent.shape)
        xEvent = spikeEvent[:,2]
        #print(xEvent)
        yEvent = spikeEvent[:,1]
        #print(yEvent[0:50])
    ...
    events = event(xEvent, yEvent, pEvent, tEvent * samplingTime)
    #print(events.y[0:50])
    return events
...
def read2Dspikes(filename, samplingTime=1000):
    '''
    ...
    #return event(xEvent, yEvent, pEvent, tEvent)
    return event(xEvent, yEvent, pEvent, tEvent / samplingTime)

#def encode2Dspikes(filename, TD):
def encode2Dspikes(filename, TD, ms):
    '''
    ...
    pEvent = np.round(TD.p).astype(int)
    tEvent = np.round(TD.t).astype(int)  # encode spike time in us
    if ms:
        tEvent = np.round(TD.t * 1000).astype(int)  # encode spike time in us
    outputByteArray = bytearray(len(tEvent) * 5)
...
def encodeNpSpikes(filename, TD, fmt='xypt', timeUnit=1e-3):
    ...
    if fmt=='xypt':
        ...
        raise Exception("fmt='%s' not implemented."%(fmt))
    print('npy array size: ', npEvent.shape)
...
def _showTD2D(TD, fig=None, frameRate=24, preComputeFrames=True, repeat=False, plot=True):
    ...
    if preComputeFrames is True:
        ...
        # NOTE: all the events in the interval `interval` have been
        # flattened into a single frame
        # precompute frames
        for i in range(len(frames)):
            ...
            #print(np.sum(frames[i,:,:,1]))
            frames[i, TD.y[bInd], TD.x[bInd], 2] = 1
            #print('channel 0')
            #print(frames[i, 20:40, 20:40, 0])
            #print('channel 1')
            #print(frames[i, 20:40, 20:40, 1])
            #print('channel 2')
            #print(frames[i, 20:40, 20:40, 2])
