...
class GestureDataset(Dataset):
    ...
    def _augment_data(frame):
        ...
        #print('Rotated coords: x: ({}, {}), y: ({}. {})'. \
        #        format(x_new.min(), x_new.max(), y_new.min(), y_new.max()))
        x_new = np.clip(x_new, 0, W-1)
        y_new = np.clip(y_new, 0, H-1)
        #x_new = np.clip(x_new, x.min(), x.max())
        #y_new = np.clip(y_new, y.min(), y.max())
...
class Topology3(torch.nn.Module):
    def __init__(self, num_channels=1):
        super(Topology3, self).__init__()
        ...
        self.conv5 = nn.Conv2d(64, 64, 3, padding=1, stride=1, bias=False) # 64,32,32
        self.drop5 = nn.Dropout(0.1)
        self.relu5 = nn.ReLU()

        self.pool6 = nn.AvgPool2d(2) # 64,16,16
        self.drop6 = nn.Dropout(0.1)
        self.relu6 = nn.ReLU()

        self.conv7 = nn.Conv2d(64, 32, 3, padding=1, stride=1, bias=False) # 32,16,16
        self.drop7 = nn.Dropout(0.1)
        self.relu7 = nn.ReLU()

        self.conv8 = nn.Conv2d(32, 32, 3, padding=1, stride=2, bias=False) # 32,8,8
        #self.drop8 = nn.Dropout(0.1)
        self.drop8 = nn.Dropout(0.2)
        self.relu8 = nn.ReLU()

        self.conv9 = nn.Conv2d(32, 32, 3, padding=1, stride=2, bias=False) # 32,4,4
        #self.drop9 = nn.Dropout(0.40)
        self.drop9 = nn.Dropout(0.42)
        self.relu9 = nn.ReLU()

        self.fc10   = nn.Linear(512, 11, bias=False)
...
if __name__ == '__main__':
    ...
    if netParams['dataset']['keep_polarity']:
        num_channels = 2
    net = Topology3(num_channels).to(device).float()
    print(net)
    ...
    optimizer = torch.optim.Adam(net.parameters(), lr = lr, amsgrad = True, weight_decay=13e-4)
    ...
    #myFuncs.visualize_data(trainingSet, outdir, prefix='train', fac=700)
